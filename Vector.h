...
class Vector {
public:
  //! \note Все конструкторы и операторы присваивания должны создавать вектор,
  //! где _capacity равно _size Конструктор без параметров
  Vector() = default;
  ...
  /*!
      ...
      если (pos + count) > size, то элементы интервала [pos, size) должны быть
     удалены
  */
  ...
  /*!
      ...
      если endPos > size, то элементы интервала [beginPos, size) должны быть
     удалены
  */
  ...
  //! Если capacity > _capacity, то выделить новый участок памяти размером
  //! capacity и перенести вектор туда, иначе - ничего
...
private:
  ...
  size_t _capacity = 0; //! максимальный размер (количество элементов, под
                        //! которое выделена память)
  float _multiplicativeCoef =
      2.0f; //! коэффициент увеличения _capacity вектора при _size == _capacity
};